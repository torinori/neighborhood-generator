...
typedef adjacency_list<vecS, vecS, undirectedS, property<vertex_index_t, int>, property<edge_index_t, int>> PlanarGraph;
...
struct TNode {
    ...
    TNode(Id id, Index index, double lat, double lng) : id(id), index(index), lat(lat), lng(lng) {}
};
...
struct TGraph {
    ...
    void print_info() {
        ...
        auto number_of_edges = std::accumulate(edges.begin(), edges.end(), 0,
...
TGraph read_graph_from_json(const std::string &path) {
    ...
    if (json_input.Parse(js.c_str()).HasParseError()) {
        std::string error_msg = std::string(rapidjson::GetParseError_En(json_input.GetParseError())) + " (offset: " +
...
int main() {
    ...
    auto graph = read_graph_from_json("../data_samples/" + input_file);
    graph.print_info();
    ...
    // Log to file.
    std::string output_file = input_file.substr(0, input_file.find_last_of('.')) + "_result.json";
